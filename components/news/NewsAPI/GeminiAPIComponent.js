...
const GEMINI_API_KEY = process.env.EXPO_PUBLIC_API_KEY; // Gemini 2.0 API 키
...
export const fetchNewsTrends = async (newsTitles) => {
    try {
        ...
        const prompt = `다음 뉴스 제목을 분석하여 가장 중요한 10가지 트렌드를 추출해줘. 

        🎯 **출력 규칙**:
        - **"1. 주제" 형식으로만 출력** (설명 없이 제목만 작성)
        - **중요도 순서대로 1~10번까지 나열**
        - **불필요한 문장 없이 오직 10개 항목만 작성**
        ...
        6. 트럼프 관세 정책 및 경제 불확실성 증가
        7. 농협 채용 및 농촌 지원 사업 확대
        8. 2025년 주요 행사 및 정책 발표 (APEC, 월드컵, 등)
        9. 군 관련 사고 및 안전 문제 (오폭, 탄약)
        10. 암호화폐 시장 변동성 및 규제 논의

        
        📌 **뉴스 제목 목록**:
        ${newsTitles.join("\n")}
        
        🔍 위 뉴스 제목을 분석하여 가장 중요한 10가지 트렌드를 위의 출력 규칙대로 제공해줘.`;
