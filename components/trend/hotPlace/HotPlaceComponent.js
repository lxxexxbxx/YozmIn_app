...
async function kakaoSearchKeyword(category, userLocation) {
    ...
    return (data.documents || []).map((d) => ({
        id: d.id, name: d.place_name, lat: parseFloat(d.y), lng: parseFloat(d.x),
        category, openNow: true, distanceKm: d.distance ? Number(d.distance) / 1000 : undefined,
    }));
}
...
async function naverTrendEnrich(places) {
    ...
    try {
        ...
        const bodyBase = {
            startDate: "2025-09-01", endDate: "2025-09-21", timeUnit: "date",
            keywordGroups: places.slice(0, 5).map((p) => ({groupName: p.name, keywords: [p.name]})),
        };
        // TODO: 네이버 데이터랩 사용가능해지면 주석제거하여 사용
        ...
        return places.map((p) => ({
            ...
            ageVector: TARGET_AGE_VECTOR,               // 실제 응답 파싱 시 교체
            gender: {male: 0.5, female: 0.5},         // 실제 응답 파싱 시 교체
            congestion: ["여유", "보통", "붐빔"][Math.floor(Math.random() * 3)], // 대체치
        }));
    } catch (e) {
...
function calcScore(p, category, includeCongestion) {
    ...
    const base = W.wR * Rn + W.wQ * Qn + W.wP * Pn + W.wT * Tn + W.wD * Dn + W.wA * An + W.wC * Cn;
...
export default function HotPlaceScreen() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const scored = list.map((p) => ({...p, score: calcScore(p, category, includeCongestion)}));
                const sorted = scored.sort(
                    (a, b) =>
                        (b.score ?? 0) - (a.score ?? 0) ||  // 1순위: 추천점수
                        (b.rating ?? 0) - (a.rating ?? 0) ||   // 2순위: 평점
                        (b.reviews ?? 0) - (a.reviews ?? 0) || // 3순위: 리뷰 수
                        (a.distanceKm ?? 0) - (b.distanceKm ?? 0) // 4순위: 가까운 거리
                );
                setPlaces(sorted);
            } catch (e) {
    ...
    /** 헤더(카테고리/필터) — FlatList의 ListHeaderComponent로 붙임 */
    ...
    ), [category, includeCongestion, openOnly]);
    ...
    const renderItem = useCallback(({item}) => (
        <View style={styles.card}>
            <View style={{flexDirection: "row", justifyContent: "space-between", alignItems: "center"}}>
                ...
                <View style={[styles.badge, {backgroundColor: congestionColor(item.congestion)}]}>
                    <Text style={styles.badgeText}>{item.congestion ?? "-"}</Text>
                </View>
            </View>
            <Text style={styles.meta}>⭐ {item.rating ?? "-"} · 리뷰 {item.reviews ?? "-"}</Text>
            <Text style={styles.meta}>인기도: {item.trend ?? "-"}</Text>
            <Text style={styles.meta}>거리: {item.distanceKm?.toFixed?.(1) ?? item.distanceKm ?? "-"} km</Text>
            <Text style={[styles.score, {color: scoreColor(item.score ?? 0)}]}>
        ...
        <View style={styles.container}>
            ...
            {loading && (
                <View style={styles.centered}>
                    ...
                    <Text style={{marginTop: 8}}>추천 계산 중…</Text>
                </View>
            )}
...
const styles = StyleSheet.create({
    ...
    card: {
        ...
        borderWidth: StyleSheet.hairlineWidth, borderColor: "#e7e7e7", backgroundColor: "#fff",
    },
